...
/** 사용자 메시지 1 + 에이전트 A 응답 1 = 1턴; 이 턴 수마다 B가 꿈을 꿈. */
const DEFAULT_TURNS_PER_DREAM = 3;
const MIN_TURNS_PER_DREAM = 1;
const MAX_TURNS_PER_DREAM = 12;
...
/**
 * 턴 구간 대화 snippet + 배치 요약 summaryText 로 꿈 체인 5회 호출 → 최종 dream 문자열
 */
async function runDreamPromptChain(apiKey, snippet, summaryText, onStep, isCancelled) {
  if (isCancelled?.()) throw new Error('cancelled');
  const userLog = `다음은 이번 턴 구간의 대화 로그다.\n\n${snippet}`;
...
/** Session 카드와 동일한 틀 안에 턴 수(숫자)만 입력 */
const TurnsCard = styled.div`
  flex-shrink: 0;
  border-radius: ${(p) => p.theme.radius}px;
  border: 1px solid ${(p) => p.theme.border};
  background: ${(p) => p.theme.surface2};
...
function App() {
  ...
  /** 통합 기억: dialogue_batch(N턴마다 1) + dream(동일 주기) 만 */
  const [memoryLog, setMemoryLog] = useState([]);
  /** 핑퐁 N번마다 memory + dream (설정 가능) */
  const [turnsPerDream, setTurnsPerDream] = useState(DEFAULT_TURNS_PER_DREAM);
  /** 입력 중 빈 칸·삭제 허용; 확정은 blur에서 turnsPerDream에 반영 */
  const [turnsDraft, setTurnsDraft] = useState(String(DEFAULT_TURNS_PER_DREAM));
  ...
  /**
   * N턴마다: 기억 스트림에 대화 요약 1개 → 이어서 꿈 1개
  ...
  useEffect(() => {
    ...
    if (agentCount <= 0 || agentCount % turnsPerDream !== 0) return;
    ...
    const batchDialogue = chatTurns.slice(-turnsPerDream * 2);
      ...
      (async () => {
        try {
          const summaryText = await callOpenAiChat(
            ...
            { logLabel: 'memory stream · 턴 구간 대화 요약(dialogue_batch)' }
          );
          if (cancelled) return;
          setMemoryLog((prev) => {
    ...
    return undefined;
  }, [chatTurns, turnsPerDream, pushActivity]);
      ...
      <Shell>
        ...
        <Main>
          <Panel>
            ...
            <PanelTitle style={{ marginTop: '0.25rem' }}>Turns per dream</PanelTitle>
            <TurnsCard>
              <TurnsInput
                type="text"
                inputMode="numeric"
                ...
                value={turnsDraft}
                onChange={(e) => {
                  setTurnsDraft(e.target.value.replace(/\D/g, ''));
                }}
                onBlur={() => {
                  if (turnsDraft === '') {
                    setTurnsPerDream(DEFAULT_TURNS_PER_DREAM);
                    setTurnsDraft(String(DEFAULT_TURNS_PER_DREAM));
                    return;
                  }
                  const v = parseInt(turnsDraft, 10);
                  if (Number.isNaN(v)) {
                    setTurnsPerDream(DEFAULT_TURNS_PER_DREAM);
                    setTurnsDraft(String(DEFAULT_TURNS_PER_DREAM));
                    return;
                  }
                  const clamped = Math.min(MAX_TURNS_PER_DREAM, Math.max(MIN_TURNS_PER_DREAM, v));
                  setTurnsPerDream(clamped);
                  setTurnsDraft(String(clamped));
                }}
                aria-label={`Turns per dream, ${MIN_TURNS_PER_DREAM} to ${MAX_TURNS_PER_DREAM}`}
              />
            </TurnsCard>
          ...
          <ChatColumn>
            <ChatHeader>
              <ChatHeaderText>
                ...
                <ChatHint>
                  Every {turnsPerDream} turns, the agent weaves one memory and one dream.
                </ChatHint>
              </ChatHeaderText>
              <HeaderActions>
